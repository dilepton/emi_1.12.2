...
import dev.emi.emi.util.FastUtilCompat;
				...
				for (int i = 0; i < ingredients.size(); i++) {
					...
					for (DiscoveredItem di : d) {
						if (biggest == null) {
							...
							int a = di.amount / (FastUtilCompat.getOrDefault(weightDivider, di.ingredient, 0) + di.consumed);
							int ba = biggest.amount / (FastUtilCompat.getOrDefault(weightDivider, biggest.ingredient, 0) + biggest.consumed);
							if (ba < a) {
								biggest = di;
							}
						}
					...
					weightDivider.put(biggest.ingredient, FastUtilCompat.getOrDefault(weightDivider, biggest.ingredient, 0) + biggest.consumed);
