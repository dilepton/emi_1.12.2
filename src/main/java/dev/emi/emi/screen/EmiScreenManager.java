...
public class EmiScreenManager {
	...
	public static void openConfigScreen() {
		client.displayGuiScreen(new ConfigScreen(client.currentScreen));
	}
	...
	public static void recalculate() {
		...
		updateSearchBarPosition(screen);
	}

	private static int resolveSearchBarWidth(GuiScreen screen) {
		if (EmiConfig.searchBarWidth > 0) {
			return EmiConfig.searchBarWidth;
		}
		return Math.min(screen.width - 80, Math.max(200, screen.width / 3));
	}

	public static void updateSearchBarPosition(GuiScreen screen) {
		if (!EmiConfig.showSearchBar || !EmiConfig.enabled) {
			search.setVisible(false);
			return;
		}
		search.setVisible(true);
		if (EmiConfig.centerSearchBar) {
			int width = resolveSearchBarWidth(screen);
			search.x = (screen.width - width) / 2;
			search.y = screen.height - 22;
			search.setWidth(width);
		} else if (EmiConfig.searchSidebar == SidebarSide.RIGHT) {
			int width = resolveSearchBarWidth(screen);
			SidebarPanel right = panels.get(1);
			if (right.space != null && right.space.tw > 0) {
				width = right.space.tw * ENTRY_SIZE;
				search.x = right.space.tx;
			} else {
				search.x = screen.width - width - 2;
			}
			search.y = screen.height - 22;
			...
			int width = resolveSearchBarWidth(screen);
			SidebarPanel left = panels.get(0);
			if (left.space != null && left.space.tw > 0) {
				width = left.space.tw * ENTRY_SIZE;
				search.x = left.space.tx;
			} else {
				search.x = 2;
			}
			search.y = screen.height - 22;
...
//				client.currentScreen.setFocused(search);
	...
	public static class SidebarPanel {
		...
		public SidebarPanel(SidebarSide side, SidebarPages pages) {
			this.side = side;
			this.pages = pages;
			pageLeft = new SizedButtonWidget(0, 0, 16, 16, 224, 0, new BooleanSupplier() {
				@Override
				public boolean getAsBoolean() {
					return SidebarPanel.this.hasMultiplePages();
				}
			}, new ButtonWidget.PressAction() {
				@Override
				public void onPress(ButtonWidget widget) {
					SidebarPanel.this.scroll(-1);
				}
			});
			pageRight = new SizedButtonWidget(0, 0, 16, 16, 240, 0, new BooleanSupplier() {
				@Override
				public boolean getAsBoolean() {
					return SidebarPanel.this.hasMultiplePages();
				}
