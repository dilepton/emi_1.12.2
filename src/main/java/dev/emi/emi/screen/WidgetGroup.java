...
import dev.emi.emi.api.widget.DrawableWidgetConsumer;
...
public class WidgetGroup implements WidgetHolder {
	...
	public void error(final Throwable e) {
		...
		widgets.add(new DrawableWidget(0, 0, width, height, new DrawableWidgetConsumer() {
			@Override
			public void render(shim.net.minecraft.client.gui.DrawContext raw, int mouseX, int mouseY, float delta) {
			}
		}).tooltip(new BiFunction<Integer, Integer, List<TooltipComponent>>() {
			@Override
			public List<TooltipComponent> apply(Integer i, Integer j) {
				List<TooltipComponent> result = Lists.newArrayList();
				for (String line : EmiUtil.getStackTrace(e)) {
					result.add(TooltipComponent.of(EmiPort.ordered(EmiPort.literal(line))));
				}
				return result;
			}
		}));
	...
	public void decorateDevMode() {
			...
			if (!recipe.getOutputs().isEmpty() && recipe.supportsRecipeTree()) {
				...
				for (Widget widget : widgets) {
					if (widget instanceof SlotWidget) {
						SlotWidget sw = (SlotWidget) widget;
		...
		if (!errors.isEmpty()) {
			...
			for (RecipeError error : errors) {
				...
				if (error.severity() == RecipeError.Severity.ERROR) {
					tooltip.add(EmiTooltipComponents.of(EmiPort.translatable("emi.dev.severity.error", Formatting.RED)));
				} else {
					tooltip.add(EmiTooltipComponents.of(EmiPort.translatable("emi.dev.severity.warning", Formatting.YELLOW)));
				}
				tooltip.addAll(error.tooltip());
			}
			final int errorColor;
			if (severity == RecipeError.Severity.ERROR) {
				errorColor = 0xCCCC0000;
			} else {
				errorColor = 0xCCCCCC00;
			}
			addDrawable(0, 0, width, height, new DrawableWidgetConsumer() {
				@Override
				public void render(shim.net.minecraft.client.gui.DrawContext raw, int mouseX, int mouseY, float delta) {
