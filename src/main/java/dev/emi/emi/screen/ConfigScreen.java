...
public class ConfigScreen extends REMIScreen {
	...
	protected void init() {
		...
		if (list != null) {
			...
			for (ListWidget.Entry e : list.children()) {
				if (e instanceof GroupNameWidget) {
		...
		this.resetButton = EmiPort.newButton(x + 2, height - 30, w / 2 - 2, 20, EmiPort.translatable("gui.done"), new ButtonWidget.PressAction() {
			@Override
		...
		this.addDrawableChild(EmiPort.newButton(x + w / 2 + 2, height - 30, w / 2 - 2, 20, EmiPort.translatable("gui.done"), new ButtonWidget.PressAction() {
			@Override
			public void onPress(ButtonWidget button) {
				ConfigScreen.this.close();
			}
		}));
		this.addDrawableChild(EmiPort.newButton(x + w / 2 + 2, height - 52, w / 2 - 24, 20, EmiPort.translatable("screen.emi.presets"), new ButtonWidget.PressAction() {
			@Override
			public void onPress(ButtonWidget button) {
				Minecraft client = Minecraft.getMinecraft();
				client.displayGuiScreen(new ConfigPresetScreen(ConfigScreen.this));
			}
		}));
		this.addDrawableChild(new SizedButtonWidget(x + w - 20, height - 52, 20, 20, 164, 0, new java.util.function.BooleanSupplier() {
			@Override
		...
		}, new java.util.function.Supplier<List<Text>>() {
			@Override
			public List<Text> get() {
				List<Text> lines = Lists.newArrayList();
				for (String line : RetroEMI.translate("tooltip.emi.config.global").split("\n")) {
					for (Object wrapped : client.fontRenderer.listFormattedStringToWidth(line, maxWidth)) {
						lines.add(Text.literal((String) wrapped));
					}
				}
				return lines;
			}
		...
		try {
			...
			Supplier<String> searchSupplier = new Supplier<String>() {
				@Override
				public String get() {
	...
	private void addJumpButtons() {
		...
		for (String s : jumps) {
			...
			final String jumpTarget = s;
			this.addDrawableChild(new ConfigJumpButton(
				2 + (newGroup ? 0 : 8), y, u, v, new ButtonWidget.PressAction() {
					@Override
					public void onPress(ButtonWidget widget) {
						jump(jumpTarget);
					}
	...
	public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
		if (activeBind != null) {
			...
			if (this.getFocused() instanceof TextFieldWidget) {
				TextFieldWidget tfw = (TextFieldWidget) this.getFocused();
				if (tfw.isFocused()) {
					if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
