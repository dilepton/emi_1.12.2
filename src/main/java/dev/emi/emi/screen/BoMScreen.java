...
public class BoMScreen extends REMIScreen {
	...
	public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
		...
		Function<EmiBind, Boolean> function = new Function<EmiBind, Boolean>() {
			@Override
			public Boolean apply(EmiBind bind) {
				return bind.matchesKey(keyCode, scanCode);
			}
		};
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		if (hover != null) {
			...
			if (hover.stack != null) {
				if (EmiInput.isShiftDown() && button == 0) {
					if (getAutoResolutions(hover, new BiConsumer<EmiIngredient, EmiRecipe>() {
						@Override
						public void accept(EmiIngredient stack, EmiRecipe recipe) {
		...
		final int clickButton = button;
		Function<EmiBind, Boolean> function = new Function<EmiBind, Boolean>() {
			@Override
			public Boolean apply(EmiBind bind) {
				return bind.matchesMouse(clickButton);
			}
		};
		if (function.apply(EmiConfig.back)) {
			EmiHistory.pop();
