...
public class IntEdit {
	...
	public IntEdit(int width, IntSupplier getter, IntConsumer setter) {
		...
		text.setChangedListener(new java.util.function.Consumer<String>() {
			@Override
		...
		text.setTextPredicate(new Predicate<String>() {
			@Override
		...
		up = new SizedButtonWidget(150, 0, 12, 10, 232, 48, new java.util.function.BooleanSupplier() {
			@Override
		...
		down = new SizedButtonWidget(150, 10, 12, 10, 244, 48, new java.util.function.BooleanSupplier() {
			@Override
			public boolean getAsBoolean() {
				return true;
			}
