...
public class EmiApi {
	...
	public static void displayRecipes(EmiIngredient stack) {
		...
		if (stack instanceof TagEmiIngredient) {
			...
			setPages(Collections.singletonMap(VanillaPlugin.INGREDIENT, Collections.singletonList(createSyntheticIngredientRecipe(stack))), stack);
	...
	private static EmiRecipe createSyntheticIngredientRecipe(final EmiIngredient ingredient) {
		return new EmiIngredientRecipe() {
			@Override
