...
public abstract class EmiIngredientRecipe implements EmiRecipe {
	...
	public void addWidgets(WidgetHolder widgets) {
		...
		if (pageSize < stacks.size()) {
			widgets.addButton(2, 2, 12, 12, 0, 0, new BooleanSupplier() {
				@Override
				public boolean getAsBoolean() {
					return true;
				}
			}, new ButtonWidget.ClickAction() {
				@Override
				public void click(double mouseX, double mouseY, int button) {
					manager.scroll(-1);
				}
			});
			widgets.addButton(widgets.getWidth() - 14, 2, 12, 12, 12, 0, new BooleanSupplier() {
				@Override
				public boolean getAsBoolean() {
					return true;
				}
	...
	private class PageSlotWidget extends SlotWidget {
		...
		public void drawBackground(DrawContext draw, int mouseX, int mouseY, float delta) {
			...
			if (BoM.getRecipe(getIngredient()) instanceof EmiResolutionRecipe) {
				EmiResolutionRecipe err = (EmiResolutionRecipe) BoM.getRecipe(getIngredient());
				if (err.stack.equals(getStack())) {
