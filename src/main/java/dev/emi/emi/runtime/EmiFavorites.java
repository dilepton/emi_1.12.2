	...
	public static void updateSynthetic(EmiPlayerInventory inv) {
		...
		if (BoM.tree != null && BoM.craftingMode) {
			...
			for (Object2LongMap.Entry<EmiRecipe> entry : batches.object2LongEntrySet()) {
				...
				long amount = FastUtilCompat.getOrDefault(amounts, recipe, 0L);
				...
				syntheticFavorites.add(new EmiFavorite.Synthetic(recipe, batch, amount, FastUtilCompat.getOrDefault(originalAmounts, recipe, amount), state));
			}
			if (!hasSomething) {
